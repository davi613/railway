...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header {
            background: linear-gradient(135deg, #FF8C00 0%, #e05c00 100%);
            padding: 40px 30px;
            text-align: center;
        }
        .header-icon {
        ...
        .warning-box strong { color: #FF8C00; }
        ...
        .footer strong { color: #FF8C00; }
        .badge {
            display: inline-block; background: #fff3e8; color: #FF8C00;
            font-size: 11px; font-weight: 600; padding: 4px 12px;
            border-radius: 20px; margin-top: 8px; border: 1px solid #ffe0c0;
        }
        .security-tips { background: #f0f9ff; border-radius: 10px; padding: 16px 20px; margin-top: 20px; }
        .security-tips p { font-size: 13px; color: #555; line-height: 1.8; }
        .security-tips ul { padding-left: 18px; margin-top: 8px; }
        .security-tips ul li { font-size: 12px; color: #666; margin-bottom: 4px; }
    </style>
</head>
<body>
<div class="wrapper">
    <div class="header">
        ...
        <p>Permintaan perubahan password diterima</p>
    </div>
    ...
    <div class="body">
        ...
        <div class="btn-wrapper">
            <a href="{{ url('/pelanggan/reset-password/' . $token . '?email=' . urlencode($pelanggan->email)) }}" class="btn-reset">
                🔑 &nbsp; Reset Password Sekarang
            </a>
        </div>

        <div class="warning-box">
            <p>⚠️ <strong>Peringatan Keamanan:</strong> Jika Anda tidak meminta reset password, segera abaikan email ini. Password Anda <strong>tidak akan berubah</strong> tanpa tindakan dari Anda. Link ini kedaluwarsa dalam <strong>60 menit</strong>.</p>
        </div>

        <div class="divider"></div>

        <p style="font-size:13px; color:#888; margin-bottom:8px;">Atau salin link berikut ke browser Anda:</p>
        <div class="link-text">{{ url('/pelanggan/reset-password/' . $token . '?email=' . urlencode($pelanggan->email)) }}</div>
